...
export function testPhysicsParameters() {
  ...
  allTestsPassed &= testParameter(
    'Gravity',
    Math.abs(physicsWorld.world.gravity.y),
    EXPECTED_PARAMETERS.gravity,
    0.1
  );
  allTestsPassed &= testParameter(
    'Timestep',
    physicsWorld.fixedTimeStep,
    EXPECTED_PARAMETERS.timestep,
    0.001
  );
  allTestsPassed &= testParameter(
    'Max Sub-Steps',
    physicsWorld.maxSubSteps,
    EXPECTED_PARAMETERS.maxSubSteps
  );
  allTestsPassed &= testParameter(
    'Solver Iterations',
    physicsWorld.world.solver.iterations,
    EXPECTED_PARAMETERS.solverIterations
  );

  // Test default sleep parameters
  console.log('\nTesting Default Sleep Parameters:');
  allTestsPassed &= testParameter(
    'Default Sleep Speed Limit',
    physicsWorld.world.defaultSleepSpeedLimit,
    EXPECTED_PARAMETERS.sleepSpeedLimit,
    0.001
  );
  allTestsPassed &= testParameter(
    'Default Sleep Time Limit',
    physicsWorld.world.defaultSleepTimeLimit,
    EXPECTED_PARAMETERS.sleepTimeLimit,
    0.001
  );
  ...
  allTestsPassed &= testParameter(
    'Sphere Linear Damping',
    testSphereBody.linearDamping,
    EXPECTED_PARAMETERS.linearDamping,
    0.001
  );
  allTestsPassed &= testParameter(
    'Sphere Angular Damping',
    testSphereBody.angularDamping,
    EXPECTED_PARAMETERS.angularDamping,
    0.001
  );
  allTestsPassed &= testParameter(
    'Sphere Sleep Speed Limit',
    testSphereBody.sleepSpeedLimit,
    EXPECTED_PARAMETERS.sleepSpeedLimit,
    0.001
  );
  allTestsPassed &= testParameter(
    'Sphere Sleep Time Limit',
    testSphereBody.sleepTimeLimit,
    EXPECTED_PARAMETERS.sleepTimeLimit,
    0.001
  );
...
function testParameter(name, actual, expected, tolerance = 0) {
  let result = true;
  const pass = tolerance > 0 ? Math.abs(actual - expected) <= tolerance : actual === expected;
