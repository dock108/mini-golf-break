...
export function testHoleVisuals(game) {
    ...
    console.log(`Test 3 - Minimal wall count (3-4): ${wallCount >= 3 && wallCount <= 4 ? 'PASS' : 'FAIL'}, Count: ${wallCount}`);
    
    // Test 4: Verify tee marker exists and has correct components
    const teeMarkers = game.scene.children.filter(obj => 
        obj.userData && obj.userData.type === 'teeMarker');
