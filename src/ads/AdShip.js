...
export class AdShip {
  constructor(adData, shipType = 'default') {
    ...
    this.shipType =
      shipType === 'default'
        ? ['nasa', 'alien', 'station'][Math.floor(Math.random() * 3)]
        : shipType;
    ...
    console.log(
      ` -> Ship ${this.group.name} using palette: P=${this.colorPalette.primary}, S=${this.colorPalette.secondary}`
    );
  ...
  createShipMesh() {
    ...
    console.log(
      `Ship ${this.group.name} - Creating with colors: Primary=${this.colorPalette.primary}, Secondary=${this.colorPalette.secondary}`
    );
  ...
  createBannerMesh() {
    let geometry;
    const bannerWidth = 3.0; // Width for ~5:1 banner
    const bannerHeight = 0.6; // Height for ~5:1 banner
    ...
    const bannerYOffset = topY + 0.3; // Position 0.3 units above the estimated top

    const bannerRotationX = -Math.PI / 2 + Math.PI / 8; // Tilt slightly upwards (~22.5 deg)
    const bannerPosition = new THREE.Vector3(0, bannerYOffset, 0); // Use calculated offset
    ...
    console.log(
      `[AdShip] Running on native platform: ${isNative}, setting depthWrite: ${bannerDepthWrite}`
    );
  ...
  updateAd(newAdData) {
    ...
    console.log(
      `Updating ad on ship ${this.group.name} from '${this.adData.title}' to '${newAdData.title}'`
    );
    ...
    if (this.bannerMesh && this.bannerMesh.material) {
      ...
      if (this.bannerMesh.userData) {
        ...
        console.log(
          `Updated banner userData to point to new ad: ${this.adData.title}, URL: ${this.adData.url}`
        );
      }
  ...
  dispose() {
    ...
    if (this.shipBodyMesh) {
      ...
      if (this.shipBodyMesh instanceof THREE.Group) {
        ...
        if (
          this.shipBodyMesh.children.length > 0 &&
          this.shipBodyMesh.children[0].material &&
          !Array.isArray(this.shipBodyMesh.children[0].material)
        ) {
  ...
  handleAdClick(event) {
    ...
    try {
      ...
      if (
        this.adData.url === '#feedback-form' ||
        this.adData.title.toLowerCase().includes('feedback')
      ) {
