...
export class AdShipManager {
  ...
  spawnInitialShips() {
    ...
    if (this.maxShips !== 4) {
      console.warn(
        `[AdShipManager.spawnInitialShips] maxShips is currently ${this.maxShips}, but this setup requires exactly 4. Adjusting...`
      );
      this.maxShips = 4;
    }
    ...
    if (!initialAds || initialAds.length < this.maxShips) {
      console.warn(
        '[AdShipManager.spawnInitialShips] Not enough initial ads found to spawn 4 ships.'
      );
      ...
      if (initialAds.length < this.maxShips) {
        console.error(
          '[AdShipManager.spawnInitialShips] FATAL: Still not enough ads after trying random. Aborting spawn.'
        );
    ...
    for (let i = 0; i < this.maxShips; i++) {
      ...
      try {
        ...
        if (ship) {
          ...
          console.error(
            `[AdShipManager.spawnInitialShips] Failed to create ship for ad: ${adData?.title}`
          );
        }
      } catch (error) {
        console.error(
          `[AdShipManager.spawnInitialShips] Error spawning ship for ad ${adData?.title}:`,
          error
        );
      }
    }
    ...
    linearShips.forEach(ship => {
      ...
      if (availableLevels.includes(-5)) {
        ...
        console.error(
          `[AdShipManager.spawnInitialShips] Ran out of levels assigning to linear ship: ${ship.adData.title}`
        );
    ...
    orbitalShips.forEach(ship => {
      if (availableLevels.length > 0) {
        ...
        console.error(
          `[AdShipManager.spawnInitialShips] Ran out of levels assigning to orbital ship: ${ship.adData.title}`
        );
  ...
  spawnShip(adData, forceType = null) {
    ...
    try {
      ...
      console.log(
        `[AdShipManager.spawnShip] Spawning ship. Forced type: ${forceType}, Actual type: ${shipType}`
      );
      ...
      ship.adDisplayDuration =
        this.minAdDuration + Math.random() * (this.maxAdDuration - this.minAdDuration);
      ...
      if (ship.shipType === 'station') {
        ...
        const speed =
          this.minAngularSpeed + Math.random() * (this.maxAngularSpeed - this.minAngularSpeed);
      ...
      console.log(
        `Created AdShip: ${adData.title} type ${ship.shipType} mode: ${ship.movementType}`
      );
    ...
    if (this.ships.length > 0) {
      ...
      if (isForTopLevel) {
        ...
        switch (edge) {
          ...
          case 1: // Start at -X edge, move +X
            ...
            console.log(
              `[_getLinearStartPosAndVel] Non-top-level, Edge ${edge}. Set initial Z: ${position.z.toFixed(2)}`
            ); // DEBUG LOG
            break;
      ...
      switch (edge) {
        ...
        case 1: // Start at -X edge, move +X
          position.z = randPosAlongEdge; // Now guaranteed >= 0
          console.log(
            `[_getLinearStartPosAndVel] Initial setup, Edge ${edge}. Set initial Z: ${position.z.toFixed(2)}`
          ); // DEBUG LOG
          break;
        case 2: // Start at +Z edge, move -Z
          position.x = randPosAlongEdge;
  ...
  _orientShip(ship) {
    ...
    if (ship.movementType === 'orbit') {
      ...
      ship.group.lookAt(
        ship.group.position.x + tangentX,
        lookAtY,
        ship.group.position.z + tangentZ
      );
    } else if (
      ship.movementType === 'linear' &&
      ship.velocity &&
      ship.velocity.lengthSq() > 0.001
    ) {
  ...
  update(deltaTime, ballPosition = null) {
    ...
    this.ships.forEach((ship, i) => {
      ...
      const effectiveDeltaTime = deltaTime; // No slowdown adjustment needed
      ...
      if (ship.movementType === 'linear') {
        ...
        if (
          ...
          if (ship.verticalOffset === -5) {
            ...
            switch (edge) {
              case 0: // Start +X edge, move -X
                ...
                console.log(
                  `[Update Recycle] Non-top-level, Edge ${edge}. Set Z to: ${randPosAlongEdge.toFixed(2)}`
                ); // DEBUG LOG
                recycleStart.velocity.set(
                  -this.linearShipSpeed,
                  0,
                  (Math.random() - 0.5) * 0.1 * this.linearShipSpeed
                );
                break;
          ...
          // ship.slowdownTimer = 0;
          console.log(
            `Recycled linear ${ship.adData.title} to pos: (${newStart.position.x.toFixed(1)}, ${ship.verticalOffset.toFixed(1)}, ${newStart.position.z.toFixed(1)})`
          );
        }
      }
  ...
  getNextAd(currentAdTitle = null) {
    ...
    const availableAds = currentAdTitle
      ? mockAds.filter(ad => ad.title !== currentAdTitle)
      : mockAds;
