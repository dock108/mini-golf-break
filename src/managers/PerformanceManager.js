...
export class PerformanceManager {
    ...
    endTimer(name) {
        ...
        if (this.metrics[name]) {
            ...
            // Calculate average
            this.metrics[name].avg = this.metrics[name].samples.reduce((a, b) => a + b, 0) / this.metrics[name].samples.length;
    ...
    endFrame() {
        ...
        this.metrics.frameTime.avg = this.metrics.frameTime.samples.reduce((a, b) => a + b, 0) / this.metrics.frameTime.samples.length;
    ...
    getPerformanceData() {
        ...
        return {
            ...
            memory: {
                usedMB: this.metrics.memory.usedJSHeapSize ? (this.metrics.memory.usedJSHeapSize / (1024 * 1024)).toFixed(2) : 'N/A',
                totalMB: this.metrics.memory.jsHeapSize ? (this.metrics.memory.jsHeapSize / (1024 * 1024)).toFixed(2) : 'N/A'
            },
            objects: {
