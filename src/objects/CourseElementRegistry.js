...
export class CourseElementRegistry {
    ...
    registerBuiltInTypes() {
        ...
        console.log('[CourseElementRegistry] Registered built-in element types:', Object.keys(this.elementTypes));
    }
    ...
    register(elementType, ElementClass) {
        ...
        if (!ElementClass.prototype instanceof BaseElement) {
            console.warn(`[CourseElementRegistry] Element class for "${elementType}" may not be a BaseElement subclass.`);
        }
    ...
    createElement(elementType, config, world, scene) {
        ...
        console.log(`[CourseElementRegistry] Created element of type "${elementType}": ${element.name} (${element.id})`);
    ...
    createAndInitializeElement(elementType, config, world, scene) {
        ...
        if (!success) {
            console.error(`[CourseElementRegistry] Failed to initialize element of type "${elementType}": ${element.name} (${element.id})`);
        }
    ...
    createElementsFromConfig(elementsConfig, world, scene) {
        ...
        for (const config of elementsConfig) {
            try {
                ...
                if (!config.type) {
                    console.error(`[CourseElementRegistry] Element config missing 'type': ${JSON.stringify(config)}`);
                    continue;
                }
